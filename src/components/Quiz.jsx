...
const Quiz = ({ account, refreshFlag }) => {
    const [questions, setQuestions] = useState([]);
    const [currentQuestion, setCurrentQuestion] = useState(0);
    const [score, setScore] = useState(0);
    const [showScore, setShowScore] = useState(false);
    const [loading, setLoading] = useState(true);
    const [latestOnChainScore, setLatestOnChainScore] = useState(null);
    ...
    if (showScore) {
        ...
        return (
            <div className="text-center p-8 bg-gray-900 rounded-xl shadow-2xl">
                ...
                <SaveScoreButton scores={[score]} account={account} onScoreUpdate={handleScoreUpdate} />
                <button
                    onClick={restartQuiz}
